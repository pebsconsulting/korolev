...
object PerformanceBenchmark extends App {
    ...
    Stream.eval(state) flatMap {
      case (outboundQueue, phase, eventCb, report) =>
        ...
        inboundStream
                ...
                reaction(message) {
                  case FunctionCall(id, _, "SetRenderNum", Ast.Num(rNum) :: Nil) =>
                    ...
                    for {
                      ...
                      cb <- eventCb.get
                      _ <- Task.schedule((), (Random.nextDouble * 1).seconds)
          ...
          .interruptWhen(report.map(_.ts.length > 50))
          .onFinalize {
  ...
  def runTest(jobNum: Int, reports: Signal[Task, List[Report]], client: HttpClient[Task]) = {
    ...
    val request = HttpRequest.get[Task](Uri.http("localhost", 8181, "/"))
        ...
        case Right(body) =>
          ...
          sessionOpt.fold(Stream.empty[Task, Unit]) {
            case KorolevSession(deviceId, sessionId) =>
              val request = WebSocketRequest.ws("localhost", 8181, s"/bridge/web-socket/$deviceId/$sessionId")
              client
  ...
  val mainTask = for {
    ...
    _ <- Task.parallelTraverse(1 to 200)(n => runTest(n, reportsSignal, client))
